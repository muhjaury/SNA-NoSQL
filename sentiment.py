...
import numpy as np
from supportprocess import textpreprocess

class sentistrength:
    def __init__(self, config=dict()):
        self.negasi = [line.replace('\n','') for line in open("sentimentassets/negatingword.txt").read().splitlines()]
        self.tanya = [line.replace('\n','') for line in open("sentimentassets/questionword.txt").read().splitlines()]
        #create sentiment words dictionary
        self.sentiwords_txt = [line.replace('\n','').split(":") for line in open("sentimentassets/sentiwords_id.txt").read().splitlines()]
...
def analyzesentiment(sentence):
    ...
    data = senti.main(sentence)
    return data

# Process the sentiment
def sentiprocess(text):
    # Analyses the sentiment
    norm_text = textpreprocess.textpreprocessSenti(str(text))
    senti_analysis = analyzesentiment(norm_text)
    senti_result = senti_analysis['kelas']

    return norm_text, senti_result

# This is just the test
if __name__ == '__main__':
    print("You run sentiment.py")

    kalimat = "saya suka kamu"

    print(kalimat)
    print(sentiprocess(kalimat))
